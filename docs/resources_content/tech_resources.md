...
Building on major European initiatives—including the DS4SSCC deployment project, the EU LDT Toolbox, Simpl Smart Middleware, and Gaia-X trust frameworks—the project provides pilots with a harmonised set of standards, protocols, and tools that enable seamless data sharing across sectors and borders. The architecture outlined in the report shows how local data platforms can progressively become data spaces through trust frameworks, discovery services, and usage-controlled data exchange, and how these data spaces can then interconnect through shared connectors to form a European-wide federation. This is the foundation upon which pilots can interconnect existing LDTs (WS1), expand services to address common local needs (WS2), and integrate advanced AI-driven capabilities (WS3).

At the operational level, the technical landscape equips pilots with concrete, open technologies—such as NGSI-LD context brokers, DCAT/DCAT-AP metadata catalogs, ODRL for usage policies, and standardized APIs—to ensure that LDT services are discoverable, reusable, and portable across cities. Combined with AI and simulation technologies supported through Testing and Experimentation Facilities (TEFs), this ecosystem enables cities to move from isolated platforms to federated, trustworthy, and citizen-centric digital twins. Ultimately, the landscape empowers communities to collaborate, innovate, and scale digital services that advance Europe’s green and digital transitions.
